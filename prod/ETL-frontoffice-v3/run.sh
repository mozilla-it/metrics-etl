...
HDFS_DEST_BASE="/data/fhr/frontoffice/3/"
...
report_and_die() {
    echo "Job(s) ${FAILED_JOBS} failed - output in ${TMPDIR}" >&2
    exit 1
}
...
if [ $? -ne 0 ]; then
    for job in $JOBS; do
        python ${SCRIPTS_PATH}/${job}_job.py \
            ...
            --no-output \
            --jobconf mapred.reduce.tasks=64 \
...
for job in $JOBS; do
    ...
    ${SCRIPTS_PATH}/load_into_vertica.sh ${TBL} ${job} > vertica_import.log 2>&1
